...
class FilesController {
  static async postUpload(request, response) {
    ...
    await fs.promises.mkdir(pathDir, { recursive: true }).catch((error) => {
      return response.status(400).send({ error: error.message });
    });

    await fs.promises.writeFile(pathFile, buff).catch((error) => {
      return response.status(400).send({ error: error.message });
    });
  ...
  static async getFile(request, response) {
    ...
    return response.sendFile(filePath, { headers: { 'Content-Type': fileMimeType } });
  }
}
